...
def validate_config(raw_config):
    ...
    _require_int(workload.setdefault("concurrency", 32), "workload.concurrency", 1)
...
class VehicleRowGenerator:
    ...
    def __init__(self, config, now_utc=None):
        ...
        self.stream_count = (
            self.vehicle_count * self.timeline_count * self.window_slot_count
        )
    ...
    def context(self, index, now_epoch_seconds=None):
        rows_per_vehicle = self.timeline_count * self.window_slot_count
        vehicle_index = (index // rows_per_vehicle) % self.vehicle_count
        within_vehicle = index % rows_per_vehicle
    ...
    def _generate_value(self, column, context, rng):
        ...
        if generator == "random_blob":
            size = column["size"]
            value = bytearray()
            for _ in ITER_RANGE(size):
                value.append(rng.getrandbits(8))
            return value
...
class LoadController:
    ...
    def claim(self):
        with self.lock:
            ...
            if (
                self.total_rows > 0
                and self.succeeded + self.in_flight >= self.total_rows
            ):
                return None
...
def _worker_guard(
    ...
    try:
        _worker(session, prepared, config, generator, controller)
...
def run_load(session, prepared, config, generator=None, show_progress=True):
    ...
    worker_state = {"remaining": concurrency, "errors": []}
...
def connect(config):
    ...
    if connection["local_dc"]:
        cluster_args["load_balancing_policy"] = DCAwareRoundRobinPolicy(
            local_dc=connection["local_dc"]
        )
...
def main(argv=None):
    ...
    try:
        ...
        print(
            ">>> Loading {} with concurrency={} temperature_source={} "
            "event_time_mode={}".format(
